"""
Management command to seed auth-service with test users and profiles.
NOTE: Cases/hearings/attachments data must be seeded via their respective services.
"""
...
class Command(BaseCommand):
    help = 'Seed auth-service with test users (judges, lawyers, citizens, admins)'

    def add_arguments(self, parser):
        parser.add_argument(
            '--clear',
            action='store_true',
            help='Clear existing non-superuser accounts before seeding',
        )
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS('Starting auth data seeding...'))
